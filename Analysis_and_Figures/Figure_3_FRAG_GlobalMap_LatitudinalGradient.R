...
frag_df_check_glad <- frag_df_full_glad[frag_df_full_glad$longitude>=30&frag_df_full_glad$longitude<=40&frag_df_full_glad$latitude>=60&frag_df_full_glad$latitude<=70,]
frag_df_check_glad <- frag_df_check_glad[,c("cover_diff", "CFI_diff", "FFI_diff", "AFI_diff", "CFI", "CFI_2020",
                                            "FFI", "FFI_2020", "AFI", "AFI_2020", "TCA_2020_ST", "cover_2020","tca_2020")]
...
map_frag_diff <- function(var){
  ...
  writeRaster(r.map, filename=paste0("D:/BackUp_PhD_ETH/ETHz_S5/Forest_FRAG/Outcome/2025.2.7/","frag_glad_5km_extent_", var, ".tif"), overwrite=T)
  toc()
}
...
fragClass6 <- raster("D:/BackUp_PhD_ETH/ETHz_S5/Forest_FRAG/Outcome/2025.2.7/frag_glad_5km_extent_fragClass6.tif")
...
data_t <- FRAG_Df_diff_filter %>%
  ...
  group_by(group) %>%
  summarize(cfi_mean = mean(CFI_diff, na.rm=T), cfi_sd = sd(CFI_diff, na.rm=T), 
            cfi_se = se(CFI_diff), 
...
ggsave("C:/Zeus/ETHz/ETHz_S5/Forest_FRAG/Results/Week34_2024.11.4_ScienceResponse/Figure3_FRAG_3Metrics_6Class_GlobalMap_RussiaCorrected.pdf", plot=MainPlot2, width=20, height=10)
