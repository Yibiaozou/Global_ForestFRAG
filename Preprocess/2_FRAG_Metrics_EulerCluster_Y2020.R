...
raster0 <- raster("/nfs/nas22.ethz.ch/fs2201/usys_ibz_cr_lab/Yibiao/FRAG/Forest_Extent/2020/00N_060W.tif")
...
setwd("/nfs/nas22.ethz.ch/fs2201/usys_ibz_cr_lab/Yibiao/FRAG/Forest_Extent/2020")
# setwd("D:/FRAG/Forest_Extent/2020_extra/")
...
for(s in 1:length(fileList)){
  ...
  results <- foreach(pair = iter(input_list),.packages=(.packages()), .combine = 'rbind') %dopar% {
    ...
    if(sum(as.matrix(habitat_raster_crop))>0){
      ...
      MPC_Df_Seg <- data.frame(cbind(longitude=lon, latitude=lat, 
                                     ...
                                     pci=pci, si=si))
    # MPC_Df_Seg <- data.frame(cbind(longitude=lon, latitude=lat, lsi=lsi, enn=enn, parmn=parmn,
    #                                pladj=pladj))
    }
  ...
  saveRDS(results, file=paste0("/cluster/home/yibzou/FRAG/Outcome/2024_12_2_FRAG_Scale/frag_df_Y2020_",rs_name,"_5km.rds"))
